...
uint64_t get_num_bytes_allocated(void *los) {
    ...
    __android_log_print(ANDROID_LOG_WARN,
                        Native_HEAP_TAG,
                        "%s,%lu", "get_num_bytes_allocated ->", num_bytes_allocated);
    return num_bytes_allocated;
}

void *los_alloc_proxy(void *thiz,
                      void *self,
                      size_t num_bytes,
    ...
    void *largeObjectMap = ((los_alloc) los_alloc_orig)(
            thiz,
            self,
            num_bytes,
            bytes_allocated,
...
void *allocate_internal_with_gc_proxy(void *heap,
                                      void *self,
    ...
    void *object = ((alloc_internal_with_gc_type) alloc_internal_with_gc_orig)(
            heap,
            self,
            allocator,
            instrumented,
    ...
    if (object == NULL && findThrowOutOfMemoryError) {
        ...
        object = ((alloc_internal_with_gc_type) alloc_internal_with_gc_orig)(
                heap,
                self,
                allocator,
                instrumented,
...
void throw_out_of_memory_error_proxy(void *heap,
                                     void *self,
                                     size_t byte_count,
    ...
    __android_log_print(ANDROID_LOG_WARN,
                        ...
                        tryAgainAllocateInternalWithGc);
    // oom发生，把oom的标志位设置为true
...
void heap_record_free_proxy(void *heap,
...
void grow_for_utilization_proxy(void *heap,
                                void *collector_ran,
...
Java_com_netease_cloudmusic_nativeheap_NativeHeapManager_heapMemoryInc(JNIEnv *env, jobject thiz) {
    //JNIEnv* env存在着大量被封装好的函数, 可以理解为一个函数映射表
    //如果方法非静态, jobject表示包含这个方法的已经实例化的类对象, 否则表示该类的引用
